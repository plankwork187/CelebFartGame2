...
const GAME = (() => {
  ...
  function update(dt) {
    ...
    FART_TRACKER.tick(dt, { power: PLAYER.getHoldPower ? PLAYER.getHoldPower() : 0 });
  ...
  function handlePlayerRelease(power, isPuff) {
    ...
    // Fart tracker: record fart end stats
    FART_TRACKER.onFartEnd({ power, isPuff, smellLevel, loudLevel });
  ...
  function reactNpc(npc, source, power, closeness = 1) {
    ...
    FART_TRACKER.onNpcDetected(npc.id || 'npc', source);
  ...
  function triggerCaught() {
    ...
    const fartSummary = FART_TRACKER.endLevel(false);
  ...
  function triggerWin() {
    ...
    running = false;
    PLAYER.stopHold();
    const fartSummary = FART_TRACKER.endLevel(true);
